...
# to send email using gmail... has been configured by config.json
app.config.update(
...
class Posts(db.Model):
    ...
    img_file = db.Column(db.String, unique=True)
...
def edit(sno):
    if 'user' in session and session['user']==params['admin_user']:
        if request.method=='POST':
            ...
            if sno=='0':
                post=Posts(title=box_title,tagline=t_line,slug=slug,content=content,img_file=img_file,date=date)
                db.session.add(post)
                db.session.commit()
...
def contact():
    if request.method=='POST':
        ...
        mail.send_message("New message from blog received from " + name,
                        sender=email,
